...
class Logger {
    constructor() {
        this.logDirectory = path.join(process.cwd(), process.env.LOG_DIR);
        this.logQueue = [];
    ...
    async ensureLogDirectory() {
        try {
            ...
            this.printToConsole(LogLevels.Error, `[${this.getLocalTime()}] [Error] Не вдалося створити теку логів: ${error.message}`);
        }
    }
    ...
    async ensureTodayLogFile() {
        ...
        try {
            ...
            try {
                ...
                this.printToConsole(LogLevels.Info, `[${this.getLocalTime()}] [Info] Створено новий файл логів: ${todayFilePath}`);
            } catch (writeError) {
                this.printToConsole(LogLevels.Error, `[${this.getLocalTime()}] [Error] Не вдалося створити файл логів: ${writeError.message}`);
            }
        }
    }
    ...
    log(message, level = LogLevels.Info, source = 'Logger') {
        const timestamp = new Date().toLocaleString("sv-SE", { timeZone: "Europe/Kiev" });
        const logEntry = `[${timestamp}] [${level}] ${message} (Source: ${source})`;
        this.logQueue.push({ timestamp, level, message: logEntry });
        this.processQueue();
    }
    ...
    async logToFile(timestamp, level, message) {
        try {
            ...
            try {
                ...
                this.printToConsole(LogLevels.Info, `[${this.getLocalTime()}] [Info] Створено новий файл логів: ${filePath}`);
            }
            await fs.appendFile(filePath, message + '\n');
        } catch (error) {
            this.printToConsole(LogLevels.Error, `[${this.getLocalTime()}] [Error] Failed to write to file: ${error.message}`);
        }
    }
